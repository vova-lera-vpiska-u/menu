...
export const createRecipeFx = createEffect(
  async ({ name, categories }: { name: string; categories: string[] }) => {
    fetch(`${url}/recipes/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const createSectionFx = createEffect(
  async ({ name, recipes }: { name: string; recipes: string[] }) => {
    fetch(`${url}/sections/`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
export const createIngredientFx = createEffect(
  async ({ name, price }: { name: string; price: string }) => {
